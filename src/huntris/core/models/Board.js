...
class Board extends EventEmitter {
    ...
    _addBlockToTable({ relPoints, point }) {
        const blockTable = this.blockTable;

        relPoints.forEach(relPt => {
            blockTable[point.y + relPt.y][point.x + relPt.x] = true;
        });
    }
    ...
    resetTables() {
        const blockTable = Array.apply(null, { length: this._rows }).map(() => false);

        for (let y = 0; y < this._rows; y++) {
            blockTable[y] = Array.apply(null, { length: this._cols }).map(() => false);
        }
    ...
    _findFullLines() {
        ...
        this.blockTable.forEach((row, i) => {
            const columnsHavingBlock = row.filter(column => column === true);
            const isFull = columnsHavingBlock.length === row.length;
    ...
    _generateEmptyRow() {
        return Array.apply(null, Array(this._cols)).map(() => false);
    }
